...
public class MainActivity extends AppCompatActivity {
    ...
    private void giveFood() {
        if (foodCount > 0) {
            foodCount--; experience += 20;
            Toast.makeText(this, "냥이에게 먹이를 줬어요! 남은 먹이: " + foodCount, Toast.LENGTH_SHORT).show();
            if (experience >= MAX_EXPERIENCE) { level++; experience -= MAX_EXPERIENCE; Toast.makeText(this, "레벨업! 현재 레벨: " + level, Toast.LENGTH_SHORT).show(); }
            updateUI(); saveData();
    ...
    private void setEditMode(boolean enabled) {
        ...
        for (int i = 0; i < farmArea.getChildCount(); i++) {
            ...
            if (child instanceof SelectableItemView) {
                ...
                if (enabled) iv.showBorderAndButtons(); else iv.hideBorderAndButtons();
            }
            if (child instanceof SelectableSpriteItemView) {
    ...
    private void showExitDialog() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("종료", (d, w) -> { saveData(); finishAffinity(); })
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("네", (d, w) -> {
                    itemView.animate().alpha(0f).scaleX(0f).scaleY(0f).setDuration(300)
                            .withEndAction(() -> { farmArea.removeView(itemView); saveAppliedItems(); })
                            .start();
                })
                .setNegativeButton("아니오", null)
                .show();
